...
def test_claim_audio_gate_depends_on_truthful_local_srmodels_apply_result():
    ...
    finish += function_body(application, "void Application::CompleteClaimProtocolActivation")
    ...
    assert "ScheduleClaimLocalAssetsRetry();" in function_body(finish, "if (!audio_service_.Start())")
    assert "return;" in function_body(finish, "if (!audio_service_.Start())")
