...
bool Open(void* raw, const char* path, tbot::LessonCinematicStreamMetadata* metadata,
    ...
    *metadata = {static_cast<std::uint16_t>(trgb ? 320 : 480),
                 ...
                 duration_ms, static_cast<std::uint32_t>(trgb ? 307200 : 64)};
...
tbot::LessonFlattenedCinematicPhaseConfig TrgbConfig(bool loop = false) {
    ...
    config.template_version = 2;
    config.phase_id = nullptr;
    config.cue_id = loop ? "barn-listen" : "barn-opening";
    config.effect = loop ? "listen" : "correct";
    config.step_key = "barn";
    config.playback_mode = loop ? tbot::LessonCinematicPlaybackMode::kLoop
                                : tbot::LessonCinematicPlaybackMode::kOnce;
    config.duration_ms = 300;
    config.frame_count = 3;
    config.loop = loop;
    config.asset.phase_id = nullptr;
    config.asset.cue_id = config.cue_id;
