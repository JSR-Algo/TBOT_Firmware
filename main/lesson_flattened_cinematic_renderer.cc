...
LessonCinematicResponse LessonFlattenedCinematicRenderer::Prepare(
    ...
    const std::size_t allocation_bytes = kFlattenedFramebufferBytes;
...
LessonCinematicResponse LessonFlattenedCinematicRenderer::Tick(std::uint64_t now_ms) {
    ...
    const bool loops = loop_ ||
        (template_version_ == 2 && playback_mode_ == LessonCinematicPlaybackMode::kLoop);
    const std::uint64_t frame = loops ? absolute_frame % metadata_.frame_count : absolute_frame;
