...
run() {
  ...
  "${compiler}" -std=c++17 -pthread -Wall -Wextra -Werror -pedantic \
    -DTBOT_WIFI_MANAGER_TESTING \
    -I"${repo_root}/tests/native_stubs/wifi_manager_recovery" \
    -I"${repo_root}/components/esp-wifi-connect/include" "$@" \
    "${repo_root}/tests/native/wifi_manager_recovery_host_test.cc" \
    "${repo_root}/components/esp-wifi-connect/wifi_manager.cc" \
    ...
    -o "${build_dir}/${label}"
  "${build_dir}/${label}"
  printf 'wifi manager recovery host test (%s) passed\n' "${label}"
}
run normal
