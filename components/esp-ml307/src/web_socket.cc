...
bool WebSocket::Connect(const char* uri) {
    ...
    connection_state_.store(ConnectionState::kHandshaking);
    xEventGroupClearBits(handshake_event_group_, HANDSHAKE_SUCCESS_BIT | HANDSHAKE_FAILED_BIT);

    // Install callbacks before the receive task can observe handshake data or failure.
    ...
    ConnectionState expected = ConnectionState::kHandshaking;
    if ((bits & HANDSHAKE_SUCCESS_BIT) &&
        connection_state_.compare_exchange_strong(expected, ConnectionState::kConnected)) {
        if (on_connected_) {
            on_connected_();
        }
        return true;
...
void WebSocket::HandleTransportDisconnected() {
    ConnectionState previous =
        connection_state_.exchange(ConnectionState::kDisconnected);
    if (previous == ConnectionState::kHandshaking) {
        xEventGroupSetBits(handshake_event_group_, HANDSHAKE_FAILED_BIT);
    } else if (previous == ConnectionState::kConnected && on_disconnected_) {
        on_disconnected_();
    }
}
